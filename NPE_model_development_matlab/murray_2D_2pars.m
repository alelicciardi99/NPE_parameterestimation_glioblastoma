...
u=@(x,y,theta) 10./(4*pi*theta(1)*t_star).*exp(theta(2)*t_star-(x.^2+y.^2)/(4*theta(1)*t_star));


[X,Y] = meshgrid(-0.25:.01:0.25,-0.25:.01:0.25);
n_points=size(X,2);

theta_obs_un=[inverse_normalizer(D_obs,range_D) inverse_normalizer(rho_obs,range_rho)];
u_obs=u(X,Y,theta_obs_un)+normrnd(0,0.1,[n_points,n_points]);
...
for i=1:N
    X_data(i,:)=reshape(u(X,Y,theta_target_un(i,:))+normrnd(0,0.1,[n_points,n_points]),...
        n_points*n_points,[]);
end
...
while (error>tol)&&(other_var<max_var2)
    ...
    X_data=zeros(N_size,n_points*n_points);
