...
%% Problem set
% this script is addressed to provide a single example for 2D spatial
% domain functions, in order to define a workspace for the
% comsol-simulation section of the work
% In this case we need to define a 2D-grid of points over which we compute
% the value of a function u(x,y,t;D) where D is the target parameter.
...
rho=0.1;
u=@(x,y,D) 10./(4*pi*D*t_star).*exp(rho*t_star-(x.^2+y.^2)/(4*D*t_star));
%Murray in-vitro tumour growth
[X,Y] = meshgrid(-2:.2:2,-2:.2:2);
n_points=size(X,2);

u_obs=u(X,Y,D_obs)+normrnd(0,0.01,[n_points,n_points]);

X_data=zeros(N,n_points*n_points);
figure(1)
histogram(D_target)
for i=1:N
    X_data(i,:)=reshape(u(X,Y,D_target(i))+normrnd(0,0.01,[n_points,n_points]),...
        n_points*n_points,[]);
end
figure(2)
surf(X,Y,u_obs)
figure(3)
contour(X,Y,u_obs)
X_obs=reshape(u_obs, n_points*n_points,[])';
s=1;
loss=1e5;
tol=0;
while (loss>tol)
    % Set up MDNetwork parameters.
    nin = n_points*n_points;			% Number of inputs.
    nhidden = 400;			% Number of hidden units.
