...
clc
rng('default');
X_test=importdata('test_data_2pars.txt');
X_test=X_test(:,3:end);
X_test=X_test';
...
theta_test=importdata('glioma_2pars_test_set.txt');
theta_test=theta_test';

theta_test_scaled=[scaler(theta_test(:,1),range_D) ...
    scaler(theta_test(:,2),range_rho)];

X_train=importdata('train_data_2pars.txt');

X_train=X_train(:,3:end);
X_train=X_train';
N=size(X_train,1);

theta_train=importdata('glioma_2pars_train_set.txt');
...
theta_train_scaled= [scaler(theta_train(:,1),range_D) ...
    scaler(theta_train(:,2),range_rho)];
...
for i=1:n_eval
    ...
    predicted_obs_val=[inverse_normalizer(mu_obs(ind,1),range_D) ...
        inverse_normalizer(mu_obs(ind,2),range_rho)];
    rescaled_error=[stan_dev_rescaler(error,range_D),...
...
table_NPE=table(NPE_results(:,1),NPE_results(:,2),NPE_results(:,3),NPE_results(:,4),...
    NPE_results(:,5),NPE_results(:,6),...
'VariableNames',{'D observed',...
    'D predicted', 'standard error D','\rho observed',...
